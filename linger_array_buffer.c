...
/** {{{ linger_array_buffer_free_object_storage */
static void linger_array_buffer_free_object_storage(
    buffer_object *intern TSRMLS_DC) {
    zend_object_std_dtor(&intern->std TSRMLS_CC);
    linger_efree(intern->buffer);
}
/** }}} */

/** {{{ linger_array_buffer_create_object */
#if PHP_MAJOR_VERSION >= 5 && PHP_MAJOR_VERSION < 7
zend_object_value linger_array_buffer_create_object(
...
zend_object *linger_array_buffer_create_object(zend_class_entry *class_type) {
    buffer_object *internal = ecalloc(
        1, sizeof(buffer_object) + zend_object_properties_size(class_type));
    zend_object_std_init(&internal->std, class_type);
    object_properties_init(&internal->std, class_type);
    internal->std.handlers = &linger_array_buffer_handlers;
...
/** {{{ linger_array_buffer_clone */
#if PHP_MAJOR_VERSION >= 5 && PHP_MAJOR_VERSION < 7
static zend_object_value linger_array_buffer_clone(zval *object TSRMLS_DC) {
    buffer_object *old_object = GET_BUFFER_OBJECT(object);
    zend_object_value new_object_val =
        linger_array_buffer_create_object(Z_OBJCE_P(object) TSRMLS_CC);
    buffer_object *new_object =
        zend_object_store_get_object_by_handle(new_object_val.handle TSRMLS_CC);

    zend_objects_clone_members(&new_object->std, new_object_val,
                               &old_object->std,
                               Z_OBJ_HANDLE_P(object) TSRMLS_CC);
...
#else if PHP_MAJOR_VERSION >= 7
static zend_object *linger_array_buffer_clone(zval *object) {
    buffer_object *old_buffer_object = GET_BUFFER_OBJECT(object);
    zend_object *new_object = linger_array_buffer_create_object(Z_OBJCE_P(object));
    buffer_object *new_buffer_object = ZOBJ_GET_BUFFER_OBJECT(new_object);
    zend_objects_clone_members(&new_buffer_object->std, Z_OBJ_P(object));

    new_buffer_object->length = old_buffer_object->length;
    if (old_buffer_object->buffer) {
        new_buffer_object->buffer = emalloc(old_buffer_object->length);
        memcpy(new_buffer_object->buffer, old_buffer_object->buffer, old_buffer_object->length);
    }
...
/** {{{ linger_array_buffer_view_free_object_storage */ 
static void linger_array_buffer_view_free_object_storage(
    buffer_view_object *intern TSRMLS_DC) {
    zend_object_std_dtor(&intern->std TSRMLS_CC);
...
/** {{{ linger_array_buffer_view_create_object */
zend_object_value linger_array_buffer_view_create_object(
    zend_class_entry *class_type TSRMLS_CC) {
    zend_object_value retval;
...
/** {{{ linger_array_buffer_view_clone */
static zend_object_value linger_array_buffer_view_clone(
    zval *object TSRMLS_DC) {
    buffer_view_object *old_object =
        zend_object_store_get_object(object TSRMLS_CC);
    zend_object_value new_object_val =
        linger_array_buffer_view_create_object(Z_OBJCE_P(object) TSRMLS_CC);
    buffer_view_object *new_object =
        zend_object_store_get_object_by_handle(new_object_val.handle TSRMLS_CC);
...
/** {{{ linger_buffer_view_offset_set */
void linger_buffer_view_offset_set(buffer_view_object *intern, long offset,
                                   zval *value) {
...
/** {{{ linger_ArrayBuffer::__construct */
PHP_METHOD(linger_ArrayBuffer, __construct) {
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &length) ==
        FAILURE) {
    ...
    zend_restore_error_handling(&error_handling TSRMLS_CC);
    if (length < 0) {
        zend_throw_exception(NULL, "Buffer length must be positive",
                             0 TSRMLS_CC);
        return;
    }
...
const zend_function_entry linger_array_buffer_methods[] = {
    PHP_ME(linger_ArrayBuffer, __construct, arginfo_buffer_ctor,
           ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
        PHP_ME(linger_ArrayBuffer, length, NULL, ZEND_ACC_PUBLIC) PHP_FE_END};

/** {{{ linger_buffer_view_get_bytes_per_element */
...
/** {{{ linger_array_buffer_view_read_dimension */
static zval *linger_array_buffer_view_read_dimension(zval *object,
                                                     zval *zv_offset,
                                                     int type TSRMLS_DC) {
    ...
    if (!zv_offset) {
        zend_throw_exception(NULL, "Cannot append to a typed array",
                             0 TSRMLS_CC);
        return NULL;
    }
    ...
    if (offset < 0 || offset >= intern->length) {
        zend_throw_exception(NULL, "Offset is outside the buffer range",
                             0 TSRMLS_CC);
        return NULL;
    }
...
/** {{{ linger_array_buffer_view_write_dimension */
static void linger_array_buffer_view_write_dimension(zval *object,
                                                     zval *zv_offset,
    ...
    offset = get_long_from_zval(zv_offset);
    if (offset < 0 || offset > intern->length) {
        zend_throw_excpetion(NULL, "Offset is outside the buffer range",
                             0 TSRMLS_CC);
        return;
    }
...
/** {{{ linger_array_buffer_view_has_dimension */
static int linger_array_buffer_view_has_dimension(zval *object, zval *zv_offset,
...
/** {{{ linger_array_buffer_view_unset_dimension */
static void linger_array_buffer_view_unset_dimension(
    zval *object, zval *zv_offset TSRMLS_DC) {
    zend_throw_exception(NULL, "Cannot unset offsets in a typed array",
                         0 TSRMLS_CC);
}
/** }}} */

/** {{{ linger_array_buffer_view_compare_objects */
static int linger_array_buffer_view_compare_objects(zval *obj1,
                                                    zval *obj2 TSRMLS_DC) {
...
/** {{{ linger_array_buffer_view_get_debug_info */
static HashTable *linger_array_buffer_view_get_debug_info(
    ...
    ZEND_INIT_SYMTABLE_EX(
        ht, intern->length + zend_hash_num_elements(properties), 0);
    zend_hash_copy(ht, properties, (copy_ctor_func_t)zval_add_ref, NULL,
                   sizeof(zval *));
...
PHP_FUNCTION(linger_array_buffer_view_ctor) {
    ...
    zend_replace_error_handling(EH_THROW, NULL, &error_handling TSRMLS_CC);
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "o|ll", &buffer_zval,
                              linger_array_buffer_ce, &offset,
                              &length) == FAILURE) {
        zend_restore_error_handling(&error_handling TSRMLS_CC);
        return;
    }
    ...
    if (offset >= buffer_intern->length) {
        zend_throw_exception(NULL,
                             "Offset has to be smaller than the buffer length",
                             0 TSRMLS_CC);
        return;
    }
    if (length < 0) {
        zend_throw_exception(NULL, "Length must be positive or zero",
                             0 TSRMLS_CC);
        return;
    }
...

    {
        size_t bytes_per_element =
            linger_buffer_view_get_bytes_per_element(view_intern);
        size_t max_length =
            (buffer_intern->length - offset) / bytes_per_element;
        if (length == 0) {
            view_intern->length = max_length;
        } else if (length > max_length) {
            zend_throw_exception(NULL, "Length is large than the buffer",
                                 0 TSRMLS_CC);
            return;
        } else {
...
const zend_function_entry linger_array_buffer_view_methods[] = {PHP_ME_MAPPING(
    __construct, linger_array_buffer_view_ctor, arginfo_buffer_view_ctor,
    ZEND_ACC_PUBLIC | ZEND_ACC_CTOR) PHP_FE_END};

static void linger_buffer_view_iterator_dtor(
    zend_object_iterator *intern TSRMLS_DC) {
...
static int linger_buffer_view_iterator_valid(
    zend_object_iterator *intern TSRMLS_DC) {
    buffer_view_iterator *iterator = (buffer_view_iterator *)intern;
    return iterator->offset < iterator->view->length ? SUCCESS : FAILURE;
}

static void linger_buffer_view_iterator_get_current_data(
    ...
    if (iterator->offset < iterator->view->length) {
        iterator->current =
            linger_buffer_view_offset_get(iterator->view, iterator->offset);
...
static void linger_buffer_view_iterator_get_current_key(
    zend_object_iterator *intern, zval *key TSRMLS_DC) {
    buffer_view_iterator *iterator = (buffer_view_iterator *)intern;
    ZVAL_LONG(key, iterator->offset);
}

static void linger_buffer_view_iterator_move_forward(
    zend_object_iterator *intern TSRMLS_DC) {
    buffer_view_iterator *iterator = (buffer_view_iterator *)intern;
    iterator->offset++;
}

static void linger_buffer_view_iterator_rewind(
    zend_object_iterator *intern TSRMLS_DC) {
    buffer_view_iterator *iterator = (buffer_view_iterator *)intern;
    iterator->offset = 0;
...
zend_object_iterator *linger_buffer_view_get_iterator(zend_class_entry *ce,
                                                      zval *object,
                                                      int by_ref TSRMLS_DC) {
    buffer_view_iterator *iterator;
    if (by_ref) {
        zend_throw_exception(NULL, "Cannot interate buffer view by refererce",
                             0 TSRMLS_CC);
        return NULL;
    }
...
PHP_MINIT_FUNCTION(linger_array_buffer) {
    ...
    memcpy(&linger_array_buffer_handlers, zend_get_std_object_handlers(),
...
#define REGISTER_ARRAY_BUFFER_VIEW_CLASS(class_name, type)                   \
    do {                                                                     \
        ...
        type##_array_ce->iterator_funcs.funcs =                              \
            &linger_buffer_view_iterator_funcs;                              \
        zend_class_implements(type##_array_ce TSRMLS_CC, 1,                  \
                              zend_ce_traversable);                          \
    } while (0)

    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\Int8Array, int8);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\UInt8Array,
                                     uint8);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\Int16Array,
                                     int16);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\UInt16Array,
                                     uint16);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\Int32Array,
                                     int32);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\UInt32Array,
                                     uint32);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\FloatArray,
                                     float);
    REGISTER_ARRAY_BUFFER_VIEW_CLASS(Linger\\ArrayBufferView\\DoubleArray,
                                     double);

    memcpy(&linger_array_buffer_view_handlers, zend_get_std_object_handlers(),
           sizeof(zend_object_handlers));
    linger_array_buffer_view_handlers.clone_obj =
        linger_array_buffer_view_clone;
    linger_array_buffer_view_handlers.read_dimension =
        linger_array_buffer_view_read_dimension;
    linger_array_buffer_view_handlers.write_dimension =
        linger_array_buffer_view_write_dimension;
    linger_array_buffer_view_handlers.has_dimension =
        linger_array_buffer_view_has_dimension;
    linger_array_buffer_view_handlers.unset_dimension =
        linger_array_buffer_view_unset_dimension;
    linger_array_buffer_view_handlers.compare_objects =
        linger_array_buffer_view_compare_objects;
    linger_array_buffer_view_handlers.get_debug_info =
        linger_array_buffer_view_get_debug_info;
    return SUCCESS;
}

PHP_MSHUTDOWN_FUNCTION(linger_array_buffer) { return SUCCESS; }

PHP_RINIT_FUNCTION(linger_array_buffer) { return SUCCESS; }

PHP_RSHUTDOWN_FUNCTION(linger_array_buffer) { return SUCCESS; }
